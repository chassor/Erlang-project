...
%% API
-export([start_link/0, makeItCrash/1, buffer/0, run/0]).
...
-record(master_state, {result_Tuple ,nodes_Map,guiPid,guiName,highestScore,first_run,bufferPid,monitorPid,inputList,processes_Map ,processes_num}).
...
handle_cast({start,Sensors ,Actuators ,Layers,  Neurons ,AF2 ,NN,Nodes}, State = #master_state{}) ->
  io:format("master start new simulation ~n"),
  ...
  if
      ...
      insert_cast(maps:to_list(MapE),HighScore),
  ...
  {noreply, State#master_state{nodes_Map = MapE, highestScore = HighScore,
    bufferPid = BufferPid,monitorPid = MonitorPid,inputList = Inputs}};
...
handle_cast({Pop_name,new_gen_hit_me,Result,Processes}, State = #master_state{ highestScore = Score ,nodes_Map = Nodes,
  bufferPid = BufferPid,inputList = Inputs,processes_Map = Processes_Map,processes_num = OldProNum}) ->
  Processes_MapNew = maps:put(Pop_name,Processes,Processes_Map),
  Number_of_processes = lists:sum(maps:values(Processes_MapNew)),
  if
    OldProNum =:=Number_of_processes -> ok;
    true -> wx_object:cast(gui_nn,{newProcessesNumber,Number_of_processes})
  end,
  ...
  {noreply, State#master_state{highestScore = Score2,processes_Map = Processes_MapNew,processes_num = Number_of_processes}};
...
handle_cast({stop},State = #master_state{nodes_Map = Nodes,bufferPid = BufferPid,monitorPid = MonitorPid}) ->
  io:format("master stop the simulation ~n"),
  BufferPid ! kill,
  MonitorPid ! kill,
...
terminate(Reason, #master_state{nodes_Map = Nodes,bufferPid = BufferPid,monitorPid = MonitorPid}) ->
  try
    BufferPid ! kill of
    _Result->ok
  catch
  ...
  L = maps:to_list(Nodes),
  stopProcess(L),
  io:format("Master terminated. The reason : ~p ~n",[Reason]),
  ok.
...
startChat([Address|T],Map,BadList,Main_PID ,SensorNum ,ActuatorNum,NumOfLayers, NumOfNeuronsEachLayer,AF,Num_Of_NN_AGENTS,Inputs ) ->
  ...
  try
     Answer = rpc:call(Address, population, start_link, [Main_PID,SensorNum,ActuatorNum,NumOfLayers,NumOfNeuronsEachLayer,AF,Num_Of_NN_AGENTS,Inputs,PopulationID]),
      if
        is_pid(Answer)->
          io:format("master : rpc succseed : ~p ~n" ,[Address]),
         ...
         _Reason:_Reason1->
           io:format("catch error while try to rpc node ~p  ~n",[Address]),
...
trytoconnect([],L)->
  if
    length(L) =:= 0 ->
      io:format("master have good connections to all nodes ~n"),
      {true,L};
    true -> {flase,L}
  end;
trytoconnect([H|T],L) ->
